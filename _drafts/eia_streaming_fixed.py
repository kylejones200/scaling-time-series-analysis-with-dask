...
def main(plot: bool = False):
    ...
    logger.error(
        "Online one-step MAE:", mean_absolute_error(ytrue_s.values, yhat_s.values)
    )
    ...
    if plot:
        ...
        y_min, y_max = (
            (min(s.min(), f.min()) if len(f) else s.min()),
            (max(s.max(), f.max()) if len(f) else s.max()),
        )
        ...
        ax.set_title(
            "EIA Net Generation — Online SARIMAX one-step forecast Jan–Aug 2025"
        )
        ax.set_xlabel("")
        ax.set_ylabel("")
